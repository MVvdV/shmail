...
- **No Inspector Surface Rule**: Mutation-log infrastructure may remain as source-of-truth plumbing, but dedicated mutation-inspector UI and user-facing bindings are removed. Recovery must stay on normal mail surfaces.
...
- **Label Identity Preservation**: Label metadata edits (name, color, visibility) must never sever `message_labels` associations. Only explicit label deletion may remove message/label joins.
- **Label Color Editing Contract**: All labels may be recolored locally, including system and app-critical labels. Only user-created labels may rename or delete. Non-user labels must keep their fixed names.
...
- **Sync Vocabulary Contract**: User-facing reconciliation language is `Sync`, not `Get Mail`. Sync means replay eligible local mutations and then fetch provider updates.
- **Retry Scope Contract**: User-facing retry is narrowed to `Retry Send` for failed queued-send/outbox drafts only. Generic thread-level retry affordances are removed.
...
- **User-Centric Keybindings**: Bindings are defined in a Pydantic configuration model and persisted to `config.toml`, allowing users to override defaults while maintaining universal support for Vim (j/k, g/G) and standard keys. Core read-only flows should prefer single-character bindings; `Tab` toggles the main panes while thread-viewer internal traversal keeps its own deterministic cycle.
- **Attachment Download Contract**: Attachment support is metadata-first and direct-to-download-folder, not cache-first. Sync persists attachment metadata only; user-triggered downloads fetch and write files to the configured downloads directory. Message header attachment controls are inline, keyboard focusable, and include a `Download all attachments` option.
- **Component Styling System**: Visual control styling must be expressed through app-owned TCSS classes rather than generic Textual selectors. Form controls share one `form` family across `Input`, `Select`, and `TextArea`; inline controls and utility controls may diverge intentionally but must remain part of the same app-owned component system.
